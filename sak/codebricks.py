...
def new(name, subject, keywords=None, ci=TRAVISCI, username=None, password=None):
    ...
    fmtargs = dict(name=name, description=description,
    ...
    _, _, p = sak.github.clone("%(username)s/%(repo)s" %
                               fmtargs, username=username)
    ...
    with lib.dir.cd(repo):
        ...
        with open(README, "w") as readme:
            readme.write(
                "[%(repo)s](http://%(username)s.github.io/%(repo)s)\n" % fmtargs)
            readme.write("==\n")
            readme.write("\n")
            readme.write("%(description)s\n" % fmtargs)
            readme.write("\n")
            lib.codebricks.badges(username, repo, ci,
                                  lambda s: readme.write(s + "\n"))
            readme.write("\n")
            readme.write(
                lib.codebricks.installationinstructions(username, name))
        ...
        with lib.json.proxy("package.json", "w", object_pairs_hook=jsonhook) as npm:
            ...
            npm["repository"][
                "url"] = "https://github.com/%(username)s/%(repo)s.git" % fmtargs
        ...
        shutil.copy(lib.sak.data("codebricks", "test-js-index.js"),
                    "test/js/index.js")
        shutil.copy(lib.sak.data(
            "codebricks", "test-js-src-dummy.js"), "test/js/src/dummy.js")

        if ci == TRAVISCI:
            shutil.copy(lib.sak.data(
                "codebricks", ".travis.yml"), ".travis.yml")

        shutil.copy(lib.sak.data("codebricks", ".codeclimate.yml"),
                    ".codeclimate.yml")
        ...
        lib.bower.register(
            qualifiedname, "github.com/%(username)s/%(repo)s" % fmtargs, force=True)
...
def fork(oldrepo, name, subject, keywords=None, ci=TRAVISCI, username=None, password=None):
    ...
    with lib.dir.cd(slug):
        ...
        with open(README, "a") as readme:
            readme.write("\n")
            readme.write(
                "***( forked from [%s](https://github.com/%s) )***" % (oldslug, oldrepo))
            readme.write("\n")
        ...
        with lib.json.proxy("package.json", "w", object_pairs_hook=jsonhook) as npm:
            ...
            npm["repository"][
                "url"] = "https://github.com/%s/%s.git" % (username, slug)
            npm["keywords"] = keywords
            npm["author"] = username
            npm["bugs"]["url"] = issuespage
            npm["homepage"] = homepage
        ...
        url = lib.http.url("github.com", path="%s/%s" %
                           (username, slug), username=username, secure=True)
        ...
        lib.bower.register(qualifiedname, "github.com/%s/%s" %
                           (username, slug), force=True)
