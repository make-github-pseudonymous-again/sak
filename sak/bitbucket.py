...
def new(repository, owner=None, username=None, password=None, is_private=TRUE, scm=GIT, fork_policy=NO_PUBLIC_FORKS, name=None, description=None, language=None, has_issues=FALSE, has_wiki=FALSE):
    ...
    lib.check.OptionNotInListException(
        "fork_policy", fork_policy, FORK_POLICIES)
    ...
    url = "https://api.bitbucket.org/2.0/repositories/%s/%s" % (
        owner, repository)

    _, _, p = lib.curl.postjson(
        url, parameters, username, password, stddefault=None)
    print()
...
def group(owner, language, *repositories):
    ...
    for repository in repositories:
        new(repository, owner, username, password, is_private, scm,
            fork_policy, name, description, language, has_issues, has_wiki)
