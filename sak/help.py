...
def info(*path):
    ...
    fmtM = functools.partial(lib.pacman.format, pred=lib.predicate.disjunction(
        [inspect.isfunction, inspect.ismodule]))
...
def source(*path, linenos=False, filename=False):
    ...
    fmt = functools.partial(
        lib.source.pretty, linenos=linenos, filename=filename)
