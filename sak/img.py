...
def resize(img, out, width, height, mode='^'):
    subprocess.call(['convert', img, '-resize', '%sx%s%s' %
                     (width, height, mode), out])


def crop(img, out, width, height, gravity='Center', left='0', top='0'):
    subprocess.call(['convert', img, '-gravity', gravity, '-crop',
                     '%sx%s+%s+%s' % (width, height, left, top), out])
...
class inkscape(object):
    ...
    def convert(img, out, width, height, t, ext):
        ...
        if width is not None:
            args.extend(['--export-width', width])
        if height is not None:
            args.extend(['--export-height', height])
