...
def send ( method , url , data = None , **kwargs ) :
	...
	out , err , p = lib.curl.call( method , api( *url ) , contenttype , data = data , **kwargs )
...
def createhook ( owner , repo , url , name = "web" , content_type = "json" , secret = None , insecure_ssl = "0" , events = "push" , active = True , username = None , password = None ) :
	...
	config = lib.dict.select( locals( ) , [ "url" , "content_type" , "secret" , "insecure_ssl" ] )

	parameters = lib.dict.select( locals( ) , [ "name" , "config" , "events" , "active" ] )
