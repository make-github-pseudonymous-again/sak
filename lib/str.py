...
NAME_RESOLVER = (
    # MATCH
    lambda stringset, alter, string: [
        x for x in stringset if alter(x) == string],
    # PREFIX
    lambda stringset, alter, string: [
        x for x in stringset if alter(x).startswith(string)],
    # SUFFIX
    lambda stringset, alter, string: [
        x for x in stringset if alter(x).endswith(string)],
    # SUBSTR
    lambda stringset, alter, string: [
        x for x in stringset if string in alter(x)]
)

NAME_RESOLVER2 = (
    # INSIDE
    lambda stringset, alter, string: [
        x for x in stringset if inside(string, alter(x))],
)
