...
class FTP(ftplib.FTP):
    ...
    def ls(self, path='.'):
        if path == '..':
            path = '../'
        d = []
        self.retrlines('LIST %s' % path, lambda x: d.append(
            (x[0], ''.join(x.split()[8:]))))
        return d

    def loginprompt(self, config):
        print(self.connect(config['host']))
        username = config['username'] if 'username' in config else input(
            'Username for \'ftp://%s\' : ' % (config['host']))
        password = getpass.getpass(
            'Password for \'ftp://%s@%s\' : ' % (username, config['host']))
